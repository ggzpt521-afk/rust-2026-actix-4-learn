...
// ========== 6. 把 Actix 错误自动转成 MyError ==========
impl From<actix_web::error::Error> for MyError {
    // 1.1 &self 不存在 → 因为 From 是 **关联函数**，不是方法
    // 1.2 from(err) → 输入一个 Actix 错误，输出一个 MyError
...
// ========== 7. 把 SQLx 错误自动转成 MyError ==========
impl From<sqlx::Error> for MyError {
    // 2.1 from(err) → 输入一个 SQLx 错误，输出一个 MyError
...
// ========== 8. 一键使用（? 运算符自动转换） ==========
pub async fn demo() -> Result<String, MyError> {
    // 6.1 ? 运算符：如果 Err → 自动转成 MyError 并提前返回
    // 这里模拟数据库错误
    Err(MyError::DbError("连接超时".to_string()))
}
